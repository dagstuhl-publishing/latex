...
class ParseEntries
{
    ...
    protected $entries;
    protected $count;
    protected $fieldExtract;
    protected $removeDelimit;
    protected $expandMacro;
    protected $parseFile;
    protected $outsideEntry;
    protected $fid;
    protected $file;
    ...
    protected $logEntries = false; // NOTE: Can be set in frontend!

    /**
     * ParseEntries constructor.
     */
    //public function __construct(OutputHandler $out = NULL, $logEntries = false)
    public function __construct($logEntries = false)
    {
     ...
     * @param $value
     */
    public function setExpandMacro($value)
    {
        $this->expandMacro = $value;
    }
     ...
     * @param string $msg
     */
    private function writeLog(string $channel, string $msg)
    {
     ...
     * @param string|null $channel
     */
    public function clearLog($channel = NULL)
    {
        if ($channel === NULL) {
            $this->log = [];
        }
    ...
    function openBib($file)
    {
        ...
        if ($this->logEntries) {
            $this->writeLog(self::LOG_OUTPUT, count($lines). ' lines\n');
        }
     ...
     */
    function loadBibtexString(string $bibtex_string)
    {
    ...
    function getLine()
    {
        if($this->parseFile)
        {
            if(isset($this->file[$this->lineCounter]))
            {
                $line = $this->file[$this->lineCounter];
                $this->lineCounter++;
     ...
     */
    function extractStringValue(string $string)
    {
...
// echo "**** ";print_r($array);echo "<BR>";
    ...
    function reduceFields($oldString)
    {
        ...
        // 28/11/2021 - Michael Didas
        // replaced the old method for determining the keys (sse below)
        // the old method yields incorrect results, if the value of a field is part of the key of an earlier field
        // concrete example: author={....}, journal=or, -> the "or" of author will erroneously be replaced!

        $oldStringCopy = $oldString;
        ...
        foreach($values as $value) {
            ...
            $keys[] = str_replace('=', '', $parts[0]);
            unset($parts[0]);
            $oldString = implode('=,', $parts);
        }

        /**** old method for determining the keys: ********/

        $oldString = $oldStringCopy;
        foreach($values as $value)
        {
            $pos = strpos($oldString, $value);
            $oldString = substr_replace($oldString, '', $pos, strlen($value));
        ...
        if($rev[0] != ',')
            $oldString .= ',';
        $oldMethodKeys = preg_split("/=,/", $oldString);
        ...
        array_pop($oldMethodKeys);

        /**** end of old method *****/

        if (count(array_diff($keys, $oldMethodKeys)) > 0) {
            $this->writeLog(
                self::LOG_OUTPUT,
                'CAUTION: diff in keys detected! - New method: '.
                implode('/', $keys).' -- Old method: '.
                implode('/',$oldMethodKeys)
            );
        }
...
// echo "**** ";print_r($this->entries[$this->count]);echo "<BR>";
    ...
    {
        if ($this->logEntries) {
            $this->writeLog(self::LOG_OUTPUT, 'ENTRY: '. $entry);
        }
     ...
     */
    function parseEntry(string $entry)
    {
        $count = 0;
        $lastLine = FALSE;
        if(preg_match("/@(.*)([{(])/U", preg_quote($entry), $matches))
        {
            ...
            else
            {
                if($this->fieldExtract) {
                    // try to handle typical syntax errors
                    if (strpos($entry, ',') === false) {
                        $pos = strripos($entry, '}');
                        $start = substr($entry, 0, $pos);
                        $entry = $start.', ___bib_parser_error___={missing "," in entry}'.substr($entry, $pos);
                    }

                    if (strpos($entry,'=') === false) {
                        $entry = preg_replace('/\}$/s', '___bib_parser_error___={missing "=" in entry}}', $entry);
                    }
     ...
     */
    function removeDelimiters(string $string)
    {
        if($string  && ($string[0] == "\""))
        {
     ...
     */
    function explodeString(string $val)
    {
     ...
     */
    function closingDelimiter(string $val, string $delimitEnd)
    {
...
// echo "--> $bracelevel, $openquote";
    ...
    /**
     * @param string $string
     * @param bool $inpreamble
     * @return string
     * Remove enclosures around entry field values.  Additionally, expand macros if flag set.
     */
    function removeDelimitersAndExpand(string $string, bool $inpreamble = FALSE)
    {
    ...
    /**
     *  This function extracts entries taking into account how comments are defined in BibTeX.
    ...
    function returnArrays()
    {
        ...
        if($this->fieldExtract)
        {
            ...
            foreach($strings as $value)
            {
                ...
                if ($this->logEntries) {
                    $this->writeLog(self::LOG_OUTPUT, $value);
                }
